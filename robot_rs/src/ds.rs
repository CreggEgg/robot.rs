...
use wpilib_hal::{
    HAL_ControlWord, HAL_GetControlWord, HAL_GetErrorMessage, HAL_RefreshDSData,
    HAL_SendConsoleLine,
};
...
pub fn robot_log(msg: String) {
    ...
    if status != 0 {
        log::warn!("Failed to log to driver station: {:?}", unsafe {
            std::ffi::CStr::from_ptr(HAL_GetErrorMessage(status))
        });
    }
}
